...
/** The line this app writes for a gig.
 *
 *  The client's name is the point. Every line used to read "Magic show", so a
 *  note with three of them told the household there were three shows and
 *  nothing else -- not whose, and not, once two fell in the same week, which
 *  was which.
 */
export function gigLine(dateISO, startTime, clientName) {
  const when = prettyTime(startTime);
  const who = String(clientName || '').trim();
  return `${humanDate(dateISO)} Magic show${who ? ` for ${who}` : ''}${when ? ` at ${when}` : ''}`;
}

/** Is this line still exactly as this app wrote it?
 *
 *  The note is a shared document that Shine's wife edits directly, so a line
 *  she wrote, or one Shine reworded, must never be silently rewritten by a
 *  program. Only a line still in the precise shape this app emits is ours to
 *  change.
 *
 *  Deliberately NOT a loose pattern. The first attempt matched
 *  "<date> Magic show( for .+)?( at .+)?" and, with lazy quantifiers
 *  backtracking, happily claimed "Sep 19th Magic Show for the Wilsons — Nadia
 *  driving" as its own and overwrote it. Anything after "at" must look like a
 *  clock and nothing else.
 */
function isOurGigLine(raw, dateISO, clientName) {
  const t = String(raw || '').trim();
  const base = `${humanDate(dateISO)} Magic show`;
  const who = String(clientName || '').trim();
  const candidates = who ? [`${base} for ${who}`, base] : [base];
  for (const c of candidates) {
    if (t.toLowerCase() === c.toLowerCase()) return true;
    if (t.toLowerCase().startsWith(`${c.toLowerCase()} at `)) {
      const rest = t.slice(c.length + 4).trim();
      if (/^\d{1,2}(:\d{2})?\s*(am|pm)?$/i.test(rest)) return true;
    }
  }
  return false;
}

/** Write the gig line for a date: replace ours if it is there, else insert.
 *
 *  This is what lets a time arrive late. A gig is usually booked before anyone
 *  knows when it starts, and the start time turns up later with the completed
 *  questionnaire; the household should not have to be told separately.
 *
 *  Returns the text unchanged when there is nothing to do, so a caller can
 *  compare and skip the write.
 */
export function upsertGigLine(text, dateISO, line, clientName) {
  const lines = String(text || '').split('\n');
  const idx = lines.findIndex((l) => isOurGigLine(l, dateISO, clientName));
  if (idx === -1) return insertByDate(text, dateISO, line);
  if (lines[idx].trim() === line) return String(text || '');
  lines[idx] = line;
  return lines.join('\n');
}
