...
(function () {
  ...
  // A dated event whose title marks Shine as unavailable that day -- travel
  // ("Barcelona trip"), time off, or an explicit "(not available)" style flag.
  // Distinct from STANDING_KEYWORDS above: those are undated directives; this
  // tags an ordinary dated/ranged event so Check Date can show an "away"
  // warning on those days instead of a green "no gig booked" all-clear (a green
  // all-clear on a day he is in Barcelona is worse than no check at all).
  //
  // Whole-word matched so "office" never trips "off" and "trips" over prose is
  // avoided; birthdays ("Nadia's bday") and dinners never match, which is the
  // point -- only genuine away/blocked days should turn the banner amber.
  const AWAY_WORDS = [
    'trip', 'travel', 'travelling', 'traveling', 'vacation', 'holiday',
    'holidays', 'away', 'abroad', 'overseas', 'unavailable', 'blocked',
    'ooo', 'na',
  ];
  const AWAY_PHRASES = [
    'out of town', 'out-of-town', 'not available', 'day off', 'off day',
    'on leave', 'time off',
  ];

  function isAway(title) {
    const t = String(title || '').toLowerCase();
    if (AWAY_PHRASES.some(function (p) { return t.indexOf(p) !== -1; })) return true;
    return AWAY_WORDS.some(function (w) {
  ...
  // Years are never written in the note. Each date is anchored to TODAY rather
  // than to the line before it: a date is this year unless it sits comfortably
  // in the past (GRACE_PAST_DAYS), in which case it means next year's occurrence
  // -- a Dec date read in January, or a whole list carried into the new year.
  //
  // The old logic assumed the note ran in strict date order and rolled the year
  // forward on any backwards step. Real notes are not sorted: Shine's had two
  // "Nov 7" lines sitting above the August/September entries, which flipped
  // everything after them to next year, so checking Oct 10 found the Barcelona
  // trip filed under the wrong year and reported the day free. Anchoring to
  // today makes each line's year independent of the lines around it.
  ...
  function parseEvents(text, today) {
    ...
    (text || '').split('\n').forEach(function (raw) {
      ...
      // Anchor to today. Start at this year; if that occurrence is comfortably
      // past, it means next year. (validOn guards Feb 29 on a non-leap year.)
      let year = baseYear;
      if (!validOn(year, month, day)) {
        if (validOn(year + 1, month, day)) year += 1; else return; // e.g. Feb 30
      }
  ...
  window.FamilyNote = {
    ...
    hasGigOn: hasGigOn, insertByDate: insertByDate, isAway: isAway,
  };
})();
