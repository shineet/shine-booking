// api/send-invoice.js
// Sends the invoice as a PDF email attachment via Resend
// Body: { invoiceData: {...}, toEmail, toName, customMessage? }
...
export default async function handler(req, res) {
  ...
  if (!invoiceData || !toEmail) {
    return res.status(400).json({ error: 'Missing required fields: invoiceData, toEmail' });
  }
  ...
  try {
    ...
    const defaultMsg = `Please find your invoice attached for the ${invoiceData.eventName || 'upcoming event'}.\n\nA ${deposit}% deposit ($${depositAmt}) is required to secure your date. See the payment methods on page 2 of the invoice.\n\nLooking forward to an unforgettable performance!`;

    const emailBody = customMessage || defaultMsg;
    ...
    res.status(200).json({ success: true, emailId: result.id, filename });
  } catch (err) {
...
// ── PDF builder (same as generate-invoice.js) ──────────────────────────────
function buildInvoicePDF(data) {
  return new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica').fontSize(11).fillColor(DARK)
       .text('texasmentalist.com', 50, 290)
       .text('2020shine@gmail.com', 50, 307);
