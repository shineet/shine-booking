...
// Generates a branded invoice PDF for Shine, The Mentalist
// Returns: { pdf: '<base64>', filename: 'Invoice_STM-2026-XXX.pdf' }
...
export default async function handler(req, res) {
  ...
  const data = req.body;
  if (!data || !data.lineItems || !data.total) {
  ...
  try {
    ...
    const filename = `Invoice_ShineTheMentalist_${(data.clientCompany || data.clientName || 'Client').replace(/\s+/g, '_')}.pdf`;

    res.status(200).json({ pdf: base64, filename });
  } catch (err) {
...
function buildInvoicePDF(data) {
  return new Promise((resolve, reject) => {
    ...
    y += 16;
    if (data.contactName) {
      doc.font('Helvetica').fontSize(10).fillColor(DARK).text(data.contactName, 50, y);
    }
    ...
    y += 14;
    if (data.clientCity) {
      doc.font('Helvetica').fontSize(10).fillColor(DARK).text(data.clientCity, 50, y);
    }
    doc.font('Helvetica').fontSize(10).fillColor(DARK)
    ...
    y += 14;
    if (data.clientPhone) {
      doc.font('Helvetica').fontSize(10).fillColor(DARK).text(data.clientPhone, 50, y);
    }
